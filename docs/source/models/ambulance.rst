...
* ``variable_locs``: Flattened coordinates of variable bases ``[x0, y0, x1, y1, ...]``.
    * Default: user-specified
* ``spatial_beta_x``: Beta shape parameters for the X-axis (``alpha_x, beta_x``).
    * Default: (2.0, 1.0)
* ``spatial_beta_y``: Beta shape parameters for the Y-axis (``alpha_y, beta_y``).
    * Default: (1.0, 2.0)
...
This model follows a nearest-available dispatch rule with queueing and
uses an IPA-style gradient estimator for optimizing base locations.
Background reading on gradient estimators in discrete-event systems can be
found in standard IPA references.
...
* ``initial_solution``: Place each variable base at coordinates ``(10, 10)``.
